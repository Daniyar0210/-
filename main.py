...
def random_fact():
    ...
    def random_facts():
        lst = [
            "У меня проживают 3 питомцев 1 собака и кот с кошкой.",
            "Кота завут полностью Джон Сильвер мы его кратко называем джоня.",
            "Кошку зовут Баргира как понтеру из книги маунгли.",
            "Мою собаку зовут Стрелка назвали в честь собаки полетевшей в космос но порою мне кажется что стрелка ведет как 1 собака в моей жизне.",
    ]    
    return f'<p>{random.choice(factss_list)}</p>'
app.run(debug=True)
